...
def crear_app():
    ...
    def update_output(value):
        ...
        # Estos se agrupan por condición obteniendo un conteo de las drogas utilizadas por condición, posteriormente se seleccionan las 3 primeras de cada una
        ...
        for (cond, drug), count in df_2.items():
            ...
            top_drogas.append(drug)  # Esta lista nos servirá para la gráfica 3
        ...
        # Gráfico 3 - Box Plot del rating por condición solo tomando en cuenta las 3 mejores drogas por categoría
    ...
    # Definición del segundo controlador
    @app.callback(
    ...
    def update_output(value):
        ...
        # Llamado de la imputación por el val obtenido del controlador
        X_scaled_df_NAN = drop_values(df_4, val)
        data_imputada = pd.DataFrame()
        # Obtención de la data imputada con el método de interpolación spline, agregando el llenado hacia adelante para asegurar que no quede ningún valor nulo
        data_imputada = X_scaled_df_NAN.interpolate(method="spline", order=1).ffill().bfill()
        # Creación del gráfico
        figure4 = px.line(template='plotly_dark')
        # Se agregan dos líneas para cada dataframe
